...
final class Redactor {
	...
	public static function message( string $message ): string {
		...
		$message = preg_replace( '/\b(?:X-Kevira-(?:Nonce|Signature|Client-Id)|Idempotency-Key)\s*[:=]\s*[^\s,;]+/i', '[redacted-header]', $message ) ?? '';
		$message = preg_replace( '/v1=[a-f0-9]{64}/i', '[redacted-signature]', $message ) ?? '';
		$message = preg_replace( '/\b[a-f0-9]{32,}\b/i', '[redacted-token]', $message ) ?? '';
		$message = preg_replace( '/\b[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}\b/i', '[redacted-idempotency-key]', $message ) ?? '';
