...
foreach ( array( 'Plugin Name:       Kevira Mail Gateway', 'Version:           0.2.0', 'Update URI:        https://github.com/kevira-wordpress/kevira-releases', 'register_activation_hook', 'plugins_loaded' ) as $needle ) {
	if ( ! str_contains( (string) $main, $needle ) ) {
		throw new RuntimeException( 'Missing plugin contract: ' . $needle );
	}
}
if ( ! str_contains( (string) $changelog, '## [0.2.0]' ) ) {
